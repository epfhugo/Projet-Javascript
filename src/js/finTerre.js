...
export default class finTerre extends Phaser.Scene {
  constructor() {
    super({ key: "finTerre" }); // mettre le meme nom que le nom de la classe
  }
   ...
   */
  preload() {
    this.load.image("terre", "src/assets/terre.png");
...
/***********************************************************************/
  ...
  create() {
    ...
    var terre = this.add.image(920, 300, "terre").setDepth(2);
    terre.setScale(1,1);
    ...
    const meilleurScore = parseInt(localStorage.getItem(`meilleurScore_${'terre'}`)) || 0;
    const meilleureVague = parseInt(localStorage.getItem(`meilleureVague_${'terre'}`)) || 0;
    const dernierScore = parseInt(localStorage.getItem(`dernierScore_${'terre'}`)) || 0;
    const derniereVague = parseInt(localStorage.getItem(`derniereVague_${'terre'}`)) || 0;
